...
bool ResourceManager<HImage>::All_Load(const wchar_t* _Target)
{
	return ResourceManager<HImage>::All_LoadOrigin(_Target);
}
bool ResourceManager<HSound>::All_Load(const wchar_t* _Target)
{
	return ResourceManager<HSound>::All_LoadOrigin(_Target);
}
	...
	do
	{
		...
		if (true == ResourceManager<KS>::IsDot(_FD.name))
		{
				...
				Exi == L".mp3")
			{
				...
				ResourceManager<KS>::Load(_Target, Name.c_str());
			}
		}
